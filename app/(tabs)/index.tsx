...
export default function HomeScreen() {
  ...
  const fetchFreeUserCompletions = async () => {
    try {
      ...
      const allCompletions = existingCompletions
        ? JSON.parse(existingCompletions)
        : [];
  ...
  useEffect(() => {
    ...
    if (user && plan !== "free") {
      ...
      habitsSubscription = client.subscribe(
        habitChannel,
      ...
      completionsSubscription = client.subscribe(
        completionsChannel,
        (response: RealtimeResponse) => {
          if (
    ...
    try {
      ...
      if (user && plan !== "free") {
        try {
          ...
          if (habitToUpdate) {
            await databases.updateDocument(DBID!, habitCollectionId!, habitId, {
      ...
      await updateLocalStorageAfterCompletion(
        habitId,
        completionData,
        currentDate,
        newStreakCount
  ...
  const updateLocalStorageAfterCompletion = async (
    ...
    try {
      ...
      const completions = existingCompletions
        ? JSON.parse(existingCompletions)
        : [];
